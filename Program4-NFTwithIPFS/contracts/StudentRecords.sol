...
contract StudentRecords  {
    ...
    function redeemEther() public {
        ...
        if(_checkforrecord())
        {
        require(students[studentaddress].redeemed == false);
        studentaddress.transfer(1 ether);
        students[studentaddress].redeemed = true;
        }
    ...
    function registerStudent(string memory _name, uint _rollno, uint _percentage) public {
        ...
        students[msg.sender] = Student(StudentId, _name, _rollno, _percentage,false,false);
...
function _checkforrecord() internal view returns(bool) {
